...
function buildReportPrompt(params) {
  ...
  // Compress AI findings
  const aiLines = (aiFindings || [])
    .map((a) => {
      const bugs = (a.bugs || []).map((b) => `[${b.severity}] ${b.desc}`).join("; ");
      const ux = (a.ux_issues || []).map((u) => u.desc).join("; ");
      return `Step ${a.step} (${a.screenType}): ${bugs || ux || "no issues"}`;
    })
    .join("\n");
