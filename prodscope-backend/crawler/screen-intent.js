...
function detectScreenIntent(xml) {
  ...
  const signals = {
    ...
    hasOtp: has(/\botp\b|verification code|one time password|enter code|verify/),
  ...
  if (signals.hasPermission) {
    ...
    type = 'email_signup';
    confidence = 0.95;
  } else if (signals.hasPhone && !signals.hasPassword && !signals.hasOtp) {
    type = 'phone_entry';
    confidence = 0.85;
  } else if (
