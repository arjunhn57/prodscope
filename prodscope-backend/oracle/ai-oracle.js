...
async function deepCheck(screen, context) {
  ...
  try {
    ...
    console.error(`  [ai-oracle] Analysis failed for step ${screen.step}: ${e.message}`);
    return {
      ...fallback,
...
async function analyzeTriagedScreens(screens, contextData) {
  ...
  for (const screen of screens) {
    console.log(`  [ai-oracle] Analyzing step ${screen.step} (type=${screen.screenType})`);
  ...
  console.log(
    `  [ai-oracle] Analyzed ${analyses.length} screens, tokens: ${totalTokens.input_tokens}in + ${totalTokens.output_tokens}out`
  );
